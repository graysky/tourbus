...
class FavoritesCalculator
  ...
  def upcoming_shows
    if @upcoming_shows.nil?
      ...
      # ...Switch to chunking style
      Band.each_by_chunk(100, :conditions => ["bands.id in (?)", bids], :include => :upcoming_shows) do |band|
	    ...
	    OFFLINE_LOGGER.info("          found #{shows.size} show for #{band}") if shows.size > 0
