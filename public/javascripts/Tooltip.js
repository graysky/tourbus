    ...
    * @private
    ...
    _follow: function (activator, event)
    {
      ...
      activator.Tooltip.style.position = "absolute";
		...
		// Make sure the Tooltip doesn't go off the page. The 1.2 comes from Trial and error. No idea why it's needed, probably breaks in IE
		if ((left + parseInt(tooltipWidth)) > winWidth) {
			left = winWidth - parseInt(tooltipWidth) * 1.2;
		}
		// We don't track the heigh, its possible (and much more common) that the height of an item will be more than the browser pane
